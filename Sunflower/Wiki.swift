...
class Wiki {
    ...
    private func uploadableFileTypes() async {
        ...
        if let jo = await basicQuery(["meta": "siteinfo", "siprop": "fileextensions"]) {
            ...
            let exts = jo["query", "fileextensions"].arrayValue.map { $0["ext"].string! }
            
            self.valid_file_exts = Array(Set(exts.map { UTType(filenameExtension: $0)! }))
            extRegex = "(?i)\\.(\(exts.joined(separator: "|")))$"
        }
    }
